...
export default function Dashboard() {
  ...
  // Custom Days input handling
  const handleCustomDays = ({ currentTarget: input }) => {
        setCustomDays(input.value);
    };
              ...
              <div className="w-[920px] flex justify-center items-center">
                <div className="absolute z-10 left-[845px] text-white flex flex-col gap-10 ml-auto">
                  ...
                  <button className={`${days == customDays ? 'bg-[#ffc107] text-black' : 'bg-[#000000] hover:bg-[#ffc107] hover:text-black'} py-1 px-1.5 rounded-md font-medium w-8`} onClick={() => setDays(customDays)}>
                    <input type="number" id="customDays" placeholder="00" value={customDays} onChange={handleCustomDays} className="appearance-none bg-transparent border-0 px-0 py-0 m-0 w-auto rounded-lg shadow-lg text-white focus:outline-none focus:ring-0 focus:border-0 focus:p-0 focus:w-auto"/>
                  </button>
                </div>
